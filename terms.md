...
複数のルーティングプロトコルが動作していて、複数の情報源により得られた宛先ルートが複数ある場合はAD値に従い、1つの最適な宛先ルートがルーティングテーブルに登録される。そして、ある1つのルーティングプロトコルで得られた宛先ルートが複数ある場合、`メトリックの値が小さい経路`が最適な宛先ルートとしてルーティングテーブルに登録される。RIPではメトリック値にホップ数(ルータから宛先ルートまでに経由するルータの数)を使用する
