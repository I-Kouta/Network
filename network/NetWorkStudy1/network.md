...
アプリケーション同士の論理的な経路を`セッション`と言い、ノード間における論理的な経路は`コネクション`と呼ぶ。コネクションはセッション上でデータ転送を行うための論理的な経路。このコネクションは、例えばTCPコネクションのことを指す。トランスポート層のプロトコルはデータ伝送における信頼性を保証するための機能を提供しているがTCPを使用する場合で、UDPを使用する通信では信頼性はない<br><br>
